...
const MapComponent = () => {
  ...
  const onLayersChange = (list) => {
    ...
    // Check if "Building layers" is selected, then zoom in slightly
    if (list.includes('Building layers') && map) {
      const view = map.getView();
      const zoom = view.getZoom();
      view.setZoom(zoom + 3); // Zoom in by 1 level when "Building layers" is selected
      view.setCenter(fromLonLat([76.9558, 11.0168])); // Optionally center the map on a specific location
    }
  };
